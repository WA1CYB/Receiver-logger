...
import logging_receiver_rtlsdr_epy_block_0_0 as epy_block_0_0  # embedded python block
import numpy as np
import time
import threading
...
class logging_receiver_rtlsdr(gr.top_block, Qt.QWidget):

    def __init__(self, bw=600, freq_start=433500000, gain_start=45, rcvr_file="log_my_rcvr_gps.csv"):
        ...
        self.settings = Qt.QSettings("GNU Radio", "logging_receiver_rtlsdr")
        ...
        self.gps_sig_offset = gps_sig_offset = 37.5
        self.gain = gain = gain_start
        self.freq = freq = freq_start*1+1.5e3
        ...
        for c in range(3, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._gps_sig_offset_range = Range(-40, 150, 0.5, 37.5, 2000)
        ...
        for c in range(2, 3):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._gain_range = Range(0.0, 49, 0.5, gain_start, 200)
        ...
        for c in range(2, 3):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._freq_range = Range(25e6*1+88e6*0, 6e9*1+167e6*0, 100, freq_start*1+1.5e3, 20000)
        ...
        self.soapy_rtlsdr_source_0.set_frequency(0, (freq+rit*0))
    ...
    def set_freq_start(self, freq_start):
        self.freq_start = freq_start
        self.set_freq(self.freq_start*1+1.5e3)
    ...
    def set_rit(self, rit):
        ...
        self.qtgui_freq_sink_x_0.set_frequency_range((self.freq+self.rit), (self.samp_rate/4))
        self.qtgui_sink_x_0.set_frequency_range((self.freq+self.rit), self.samp_rate)
        self.soapy_rtlsdr_source_0.set_frequency(0, (self.freq+self.rit*0))
...
def argument_parser():
    ...
    parser.add_argument(
        "--gain-start", dest="gain_start", type=eng_float, default=eng_notation.num_to_str(float(45)),
...
def main(top_block_cls=logging_receiver_rtlsdr, options=None):
    if options is None:
